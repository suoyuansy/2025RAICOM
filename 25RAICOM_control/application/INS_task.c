...
//解决陀螺仪零漂问题
static void dynamic_gyro_offset_calc(fp32 gyro_offset[3], const fp32 gyro[3], const float learning_rate)
{
    for (uint8_t i = 0; i < 3; i++) {
        gyro_offset[i] -= learning_rate * gyro[i];
    }
}

static void dynamic_gyro_calibration(fp32 cali_scale[3], fp32 cali_offset[3], uint16_t *time_count)
{
    if (*time_count == 0) {
        memcpy(gyro_offset, gyro_cali_offset, sizeof(gyro_offset));
    }

    dynamic_gyro_offset_calc(gyro_offset, INS_gyro, 0.0002f);

    memcpy(cali_offset, gyro_offset, sizeof(gyro_offset));
    cali_scale[0] = cali_scale[1] = cali_scale[2] = 1.0f;
...
//    dynamic_gyro_calibration(gyro_scale_factor[0], gyro_offset, &time_count);
//    
//	for (uint8_t i = 0; i < 3; i++) {
