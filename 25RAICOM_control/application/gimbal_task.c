...
fp32 max_relative_angle_pitch=3.0f;//-2.35f;步兵//3.0f;哨兵
fp32 min_relative_angle_pitch=2.3f;//-2.9f;步兵//2.3f;
...
//    gimbal_init(&gimbal_control);
    ...
    while (1)
    {
        ...
        gimbal_mode_change_control_transit(&gimbal_control_1); //控制模式切换 控制数据过渡
        gimbal_feedback_update(&gimbal_control_1);             //云台数据反馈
        ...
        if (!(toe_is_error(YAW_GIMBAL_MOTOR_TOE) && toe_is_error(PITCH_GIMBAL_MOTOR_TOE) && toe_is_error(TRIGGER_MOTOR_TOE)))
        {
            ...
            {
//							if(gimbal_control_1.gimbal_yaw_motor.relative_angle>max_relative_angle_yaw+0.1)
//							{
//								if(yaw_can_set_current>0)yaw_can_set_current=0;
//							}
//							if(gimbal_control_1.gimbal_yaw_motor.relative_angle<min_relative_angle_yaw-0.1)
//							{
//								if(yaw_can_set_current<0)yaw_can_set_current=0;
//							}
//							if(gimbal_control_1.gimbal_pitch_motor.relative_angle>max_relative_angle_pitch+0.1)
//							{
//								if(pitch_can_set_current>0)pitch_can_set_current=0;
//							}
//							if(gimbal_control_1.gimbal_pitch_motor.relative_angle<min_relative_angle_pitch-0.1)
//							{
//								if(pitch_can_set_current<0)pitch_can_set_current=0;
//							}
...
#else
    feedback_update->gimbal_yaw_motor.relative_angle = motor_ecd_to_angle_change(feedback_update->gimbal_yaw_motor.gimbal_motor_measure->ecd + 4096,
                                                                                        feedback_update->gimbal_yaw_motor.offset_ecd);
#endif
...
static void gimbal_relative_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
    if (gimbal_motor == NULL)
    {
		...
		//gimbal_motor->relative_angle_set = inputdata.shoot_yaw;
...
static void gimbal_relative_angle_limit_1(gimbal_motor_t *gimbal_motor, fp32 add)
{
    if (gimbal_motor == NULL)
    {
		...
		//if(uwTick-tempTick>10)
     //{temp=inputdata.shoot_pitch;tempTick=uwTick;}
	//if(inputdata.shoot_pitch-temp>=0.001f||inputdata.shoot_pitch-temp<=-0.001f)
	//{
			//gimbal_motor->relative_angle_set = inputdata.shoot_pitch;
